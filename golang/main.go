...
func isFavoritedBy(ctx context.Context, db connOrTx, userAccount string, playlistID int) (bool, error) {
	...
	err := db.GetContext(
		ctx,
		&pid,
		"SELECT playlist_id FROM playlist_favorite where favorite_user_account = ? AND playlist_id = ? LIMIT 1",
...
func getFavoritesCountByPlaylistID(ctx context.Context, db connOrTx, playlistID int) (int, error) {
	...
	err := db.GetContext(
		ctx,
		&count,
		"SELECT favorite_count FROM playlist_favorite_count where playlist_id = ?",
		playlistID,
	)
	if err == sql.ErrNoRows {
		return 0, nil
...
func getPopularPlaylistSummaries(ctx context.Context, db connOrTx, userAccount string) ([]Playlist, error) {
	...
	if err := db.SelectContext(
		ctx,
		&popular,
		`SELECT playlist_id, favorite_count FROM playlist_favorite_count ORDER BY favorite_count DESC LIMIT 150`,
	); err != nil {
		return nil, fmt.Errorf(
